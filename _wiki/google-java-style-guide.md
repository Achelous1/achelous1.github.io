...
updated : 2021-02-03 17:28:17 +0900
...
### 1.1 용어정리
1. 클래스(class)라 함은 이타적(inclusive) 의미로 "통상적"인 클래스, 열거형 클래스, 인터페이스 또는 애너테이션유형(@interface)를 지칭한다.
...
* 각 블럭 안의 임포트문들은 이름을 ASCII 정렬순서로 정렬한다. (**참고**: '.'는 ';' 앞에 정렬되므로 임포트 *선언문들*이 ASCII 정렬 순서와 다릅니다)
...
##### 3.4.2.1 오버로딩(Overloads): 절대 분할하지 않는다
